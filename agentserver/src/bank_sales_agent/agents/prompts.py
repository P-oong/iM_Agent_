...
[우수 직원 분류 노하우 활용]
프롬프트의 [우수 직원 사례] 섹션에는 현장 베테랑 직원의 카테고리 판단 사례가
input_signals → expert_judgment 형태로 들어 있습니다.
- 고객 신호와 input_signals가 유사하면 expert_judgment의 카테고리 우선순위를 참고하십시오.
- 단, 이 사례는 "고객 신호 해석 기준"으로만 사용하십시오. 사례에 있는 사실을 고객에게 적용하지 말고,
  현재 고객 데이터에서 실제로 관측된 신호만 reasons에 적으십시오.
- 사례에 없는 카테고리도 고객 신호가 강하면 자유롭게 applicable에 포함하십시오.
...
[우수 직원 사례 - 카테고리 분류 노하우]
{expert_cases}

위 사례는 고객 신호 → 카테고리 매핑의 베테랑 직원 판단 기준입니다.
고객 신호가 input_signals와 유사하면 expert_judgment의 카테고리 우선순위를 참고하되,
reasons에는 이번 고객 데이터에서 실제로 관측된 신호만 적으십시오.
...
[우수 직원 성공/실패 사례 활용]
프롬프트 안의 [상품별 성공/실패 패턴] 섹션은 카테고리·상품군별로 우수 직원이
경험한 성공 신호(positive_signals)와 실패 신호(negative_signals)를 정리한 자료입니다.

활용 규칙:
- 성공 패턴(positive_signals)과 일치하는 고객 신호는 acceptance_probability를 올리는 근거로 사용하십시오.
- 실패 패턴(negative_signals)과 일치하는 고객 신호는 acceptance_probability를 낮추는 근거로 사용하십시오.
- 특히 최근 거절·민원·영업 피로도·이번 방문 목적과의 낮은 관련성은 반드시 확률 하향 요인으로 반영하십시오.
- 일치한 패턴은 출력의 success_pattern_matches / failure_pattern_matches 필드에 기록하십시오.
- 사례는 해석 기준일 뿐, 고객 데이터에 없는 사실을 만들기 위한 도구가 아닙니다.
      ...
      "top_products": [
        {
          ...
          "success_pattern_matches": ["사업자 대출 만기 D-90 이내", "최근 90일 사업자대출 금리 문의 2회 이상", "기존 대출 정상 상환 이력"],
        ...
        },
        {
          ...
          "success_pattern_matches": ["사업 영위 1년 이상", "신용 한도 한계 도달"],
          "failure_pattern_matches": [],
      ...
      "top_products": [
        {
          ...
          "success_pattern_matches": ["사업자성 입금 10회 이상/30일", "가맹점 결제계좌 미보유", "타행 정산 계좌 사용 중", "가맹점 정산 이전 문의 이력"],
          "failure_pattern_matches": [],
      ...
      "top_products": [
        {
          ...
          "success_pattern_matches": ["자사 사업자/신용카드 미보유", "사업비 지출 5회 이상/30일", "최근 카드 거절·민원 없음", "영업 피로도 낮음"],
          "failure_pattern_matches": ["이번 방문 목적이 카드와 무관"],
...
SPECIALIST_USER_TEMPLATE = """아래 Router 결과, 고객 데이터, 카테고리별 후보 상품, 우수 직원 성공/실패 패턴을 보고 각 카테고리마다 수락 확률이 높은 상품 1~2개를 선정하십시오.
...
[상품별 성공/실패 패턴 - 우수 직원 사례]
{outcome_patterns}

위 패턴은 "현재 카테고리에 해당하는" 베테랑 직원의 가입 성공/실패 신호 모음입니다.
- 고객 신호가 positive_signals와 일치하면 acceptance_probability 상승 근거로 사용하고 success_pattern_matches에 기록.
- 고객 신호가 negative_signals와 일치하면 acceptance_probability 하향 근거로 사용하고 failure_pattern_matches에 기록.
- 사례에 없더라도 고객 데이터에 있는 거절·민원·피로도 신호는 반드시 failure_pattern_matches에 반영하십시오.
    ...
    {{
      ...
      "top_products": [
        {{
          ...
          "success_pattern_matches": ["일치한 성공 신호 (positive_signals 중)"],
          "failure_pattern_matches": ["일치한 실패 신호 (negative_signals 중)"],
...
5. 거절, 민원, 부적합, 영업 피로 신호를 risk_or_caution과 failure_pattern_matches에 반영했는가?
6. 성공 패턴과 일치하는 신호는 success_pattern_matches에 기록했는가?
7. 근거가 약한데 확률이 과도하게 높게 산정되지 않았는가?
...
- 각 상품에는 우수 직원 사례 매칭 결과인 success_pattern_matches / failure_pattern_matches 가 포함되어 있습니다.
  staff_sales_talk와 next_action 작성 시 success_pattern_matches는 고객을 설득하는 강조 포인트로,
  failure_pattern_matches는 권유 강도를 조절하거나 사전 안내 항목으로 활용하십시오.
...
def build_router_prompt(
    ...
    expert_cases: list[dict] | None = None,
) -> str:
    """
    Args:
        customer_payload: 고객 Feature Mart 데이터
        expert_cases: 우수 직원 카테고리 분류 노하우 사례 리스트
                      (services.expert_cases.load_router_expert_cases 결과)
    """
    cases_block = (
        json.dumps(expert_cases, ensure_ascii=False, indent=2)
        if expert_cases
        else "[]"
    )
    return ROUTER_USER_TEMPLATE.format(
        customer_payload=json.dumps(customer_payload, ensure_ascii=False, indent=2),
        expert_cases=cases_block,
    )
...
def build_specialist_prompt(
    ...
    Args:
        ...
        outcome_patterns: 우수 직원 성공/실패 패턴 (현재 카테고리에 해당하는 것만 필터링하여 전달)
    """
    patterns_block = (
        json.dumps(outcome_patterns, ensure_ascii=False, indent=2)
        if outcome_patterns
        else "[]"
    )
    return SPECIALIST_USER_TEMPLATE.format(
        ...
        outcome_patterns=patterns_block,
    )
...
- 각 상품에는 우수 직원 사례 매칭 결과인 success_pattern_matches / failure_pattern_matches가 포함됩니다.
  - 상담 멘트(consulting_recommendation, sales_talk)는 success_pattern_matches를 강조 포인트로 활용하십시오.
  - failure_pattern_matches는 권유 강도 조절·사전 안내 항목으로 사용하고 next_actions의 주의사항으로 자연스럽게 녹이십시오.
